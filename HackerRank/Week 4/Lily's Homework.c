...
int countSwaps(int arr_count, int* arr, int* sorted) {
    int* index = malloc(arr_count * sizeof(int));
    int* visited = calloc(arr_count, sizeof(int));

    // Map each value to its index in sorted array
    ...
    int swaps = 0;
    for (int i = 0; i < arr_count; i++) {
        if (visited[i] || index[i] == i) continue;
