...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    // Step 1: Create a compressed leaderboard (unique ranks)
    ...
    // Step 2: Traverse player scores from last to first
    int idx = unique_count - 1;  // Start from lowest rank
    for (int i = 0; i < player_count; i++) {
        while (idx >= 0 && player[i] >= unique[idx]) {
            idx--;
        }
        result[i] = idx + 2; // +2 because rank = index+1, and idx moved back
    }
